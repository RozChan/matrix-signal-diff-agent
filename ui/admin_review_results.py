...
from core.feishu_file_delivery import DELIVERY_ORDER, deliver_task_result_files
...
def render_admin_review_results(task_dir: Path, show_downloads: Callable[[Path], None]) -> None:
    ...
    delivery = dict(meta.get("feishu_file_delivery") or {})
    st.subheader("飞书结果文件交付")
    ...
    if attachments:
        st.dataframe(pd.DataFrame([
            {
                "附件": value.get("display_name") or key,
                "状态": value.get("status", ""),
                "尝试次数": int(value.get("attempt_count") or 0),
                "飞书消息ID": value.get("message_id", ""),
                "错误": value.get("last_error", ""),
            }
            for key in DELIVERY_ORDER
            for value in [attachments.get(key) or {}]
        ]), hide_index=True, use_container_width=True)
    if delivery.get("last_error"):
        st.error(f"飞书交付错误：{delivery['last_error']}")

    if delivery.get("status") == "failed" and st.button("重新发送飞书结果文件", key=f"retry-feishu-delivery-{task_dir.name}"):
        result = deliver_task_result_files(task_dir)
        st.success("3个结果Excel已发送。" if result.get("success") else f"重新发送失败：{result.get('last_error') or '未知错误'}")
        st.rerun()
