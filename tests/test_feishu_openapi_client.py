...
def test_token_upload_and_send_file_to_chat(tmp_path: Path) -> None:
    ...
    assert "/open-apis/auth/v3/tenant_access_token/internal" in session.calls[0][0]
    assert "/open-apis/im/v1/files" in session.calls[1][0]
    assert session.calls[2][1]["params"] == {"receive_id_type": "chat_id"}
    body = session.calls[2][1]["json"]
...
def test_send_file_falls_back_to_open_id(tmp_path: Path) -> None:
    ...
    assert session.calls[2][1]["params"] == {"receive_id_type": "open_id"}
    assert session.calls[2][1]["json"]["receive_id"] == "ou_user"
