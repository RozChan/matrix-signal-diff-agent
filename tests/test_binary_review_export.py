...
from core.review_store import acquire_review_lock, create_task_meta, init_review_state, update_review_field
...
def test_field_review_updates_existing_value_and_numeric_is_system_different(tmp_path: Path) -> None:
    ...
    state = init_review_state(review_dir, "task", [_item()])
    fields = state["items"]["signal-1"]["field_reviews"]
    assert fields["信号长度"]["result"] == "different"
    assert fields["信号长度"]["decision_source"] == "system_default"
    assert fields["信号值描述"]["result"] == ""
    ...
    assert len(state["items"]["signal-1"]["field_reviews"]) == 3
...
def test_final_export_has_five_field_level_sheets(tmp_path: Path) -> None:
    ...
    create_task_meta(task_dir, "task", status="reviewing")
    item = _item()
    init_review_state(review_dir, "task", [item])
    acquire_review_lock(task_dir, "session")
    ...
    assert stats == {"人工确认不同": 1, "人工确认相同": 1, "系统判定不同": 1, "待人工确认": 0, "审核明细全量": 3}
